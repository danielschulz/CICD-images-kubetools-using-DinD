...
RUN \
    ...
    if [[ "${PYTHON3_VERSION}" != '3.8' ]]; then \
        ...
        # install Python when not v3.8 -- which ships by default
        apt-get install -y "python${PYTHON3_VERSION}" python3-dev ; fi && \
    ...
    apt-get install -y python3-apt python3-dev python3-distutils python3-pip git gettext-base unzip && \
